 ...
 *
 * This class has zero dependencies on your app code and can be safely removed later.
 */
public final class GridModelActorDebug {
    ...
    /**
     * @param reqW_direct  map[id -> requested power W]
     * @param step         current integer step (0 if unknown)
     * @param lastStep     last step index (or step if unknown)
     * @param timeSec      current time in seconds (0.0 if unknown)
     * @param verbosity    0=off, 1=summary, 2=per-train details
     * @return the same map instance, after optional SMOKE injection
     */
    public static Map<String, Double> smokeAndLog(Map<String, Double> reqW_direct,
        ...
        if (verbosity >= 1 && shouldPrintStep(step, lastStep)) {
            ...
            if (verbosity >= 2) {
                for (Map.Entry<String, Double> e : reqW_direct.entrySet()) {
                    System.out.printf("  [GMA-DETAIL] %s -> %.1f W%n", e.getKey(),
